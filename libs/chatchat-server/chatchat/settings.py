...
class PromptSettings(BaseFileSettings):
    ...
    llm_chat: dict = {
        "default": "{{input}}",
        "NER": (
            "<指令>作为NER任务的专家，对实体抽取的结果以如下的JSON格式返回\n"
            "Entities: [<抽取的实体以逗号分隔，不要出现嵌套数组，字符串以双引号包裹>]\n"
            "</指令>\n"
            "<待抽取句子>{{ question }}</待抽取句子>"
        ),
        # 只有上下文信息
        "context":
            """
        ...
        "professor":
            """
                "<指令>根据已知信息内容进行整理后回答问题，不允许添加任何其他内容，答案请使用中文。 </指令>\n"
                "<已知信息>{{ professor_context }}</已知信息>\n"
                "<问题>{{ question }}</问题>\n"
            """
        ,
        # 只有三元组信息
        "reasoning":
            """
                "<指令>根据<head,relation,tail>三元组信息，做知识推理，简洁和专业的来回答问题。如果无法从中得到答案，请说 “根据已知信息无法回答该问题”，不允许在答案中添加编造成分，答案请使用中文。</指令>\n"
                "<三元组信息> {{ information }} </三元组信息>\n"
                "<问题> {{question}} </问题>\n"
            """
        ,
        # 上下文信息 和 三元组信息
        "reasoning_context":
            """
                "<指令>根据<head,relation,tail>三元组信息，做知识推理并结合已知信息，简洁和专业的来回答问题。如果无法从中得到答案，请说 “根据已知信息无法回答该问题”，不允许在答案中添加编造成分，答案请使用中文。</指令>\n"
                "<三元组信息> {{ information }} </三元组信息>\n"
                "<已知信息>{{ context }}</已知信息>\n"
                "<问题> {{question}} </问题>\n"
            """
