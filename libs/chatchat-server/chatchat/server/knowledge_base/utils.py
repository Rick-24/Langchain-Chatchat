...
def format_professor_reference(kb_name: str, docs: List[Dict], api_base_url: str = "") -> Tuple[
    List[Dict], str, str]:
    '''
    将知识库检索结果格式化为参考文档的格式(带有专家知识)
    '''
    ...
    for inum, doc in enumerate(docs):
        ...
        if page_content.startswith('question:'):
            professor_knowledges.append(page_content.split('answer:')[1])
        else:
            context_knowledges.append(page_content)
        source_documents.append(ref)
    context = "\n".join(context_knowledges)
    professor_context = "\n".join(professor_knowledges)

    return source_documents, context, professor_context
